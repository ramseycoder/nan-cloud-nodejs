...
exports.shareFile = async (req, res) => {
    ...
    if (SO.etat) {
        ...
        if (!range) {
            ...
            const data = await fetch('http://localhost:5000/STORAGE/' + sharefile.path);
            const blob = await data.blob();
            //
            const videoStat = await promisify(stat)(video);
            console.log('videoStat', videoStat);
            const reader = FileReader();
            reader.onload = (e) => { console.log('r', e.target.result) }
            reader.readAsDataURL(new blob([videoStat], { type: videoStat.type }));
